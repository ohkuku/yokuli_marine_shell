...
class OfflineCoverageCoordinatorTest {
    @Test
    fun `check publishes separated facts only after exact index work completes`() = runTest {
        val chart = chart('a')
        ...
        val ready = coordinator.state.value as OfflineCoverageUiState.Ready
    ...
    @Test
    fun `cancel is explicit and a non cooperative late index result is ignored`() = runTest {
    ...
    @Test
    fun `route revision or package version invalidates immediately and rejects old completion`() = runTest {
        val release = CompletableDeferred<Unit>()
        val coordinator = OfflineCoverageCoordinator(
            tileIndex = LocalChartTileIndex { _, keys ->
                withContext(NonCancellable) { release.await() }
    ...
    @Test
    fun `over budget is a typed result and never a partial ready result`() = runTest {
        val coordinator = OfflineCoverageCoordinator(
            tileIndex = LocalChartTileIndex { _, keys -> keys },
            scope = this,
            maximumKeys = 20,
