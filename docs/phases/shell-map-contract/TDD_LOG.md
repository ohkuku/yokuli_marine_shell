...
- Both stories now explicitly require `EditIdle(selectedTile=settings-primary)` before asking the renderer for controls. This separates Engine state retention from Compose node propagation and remains useful regression coverage.
