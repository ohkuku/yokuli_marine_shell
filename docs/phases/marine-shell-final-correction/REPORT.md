...
Run `33940532006` then completed the Hosted Macrobenchmark journey step successfully; the remaining failure was only the strict post-processing summary. Since its plain `SystemExit` did not expose the missing/empty result through public Checks, the summarizer now emits an explicit GitHub error annotation while retaining the same failure. Final hosted acceptance remains pending the annotated run and any evidence-driven correction.

Annotated run `33941553833` isolated the strict summary failure to zero target frames in `startVerticalScroll60Tiles`. Its screen-relative swipe could land outside the actual grid on the Hosted Pixel 2. The journey now derives both endpoints from the `start-grid` semantics bounds and continues to measure three vertical swipes over sixty tiles; no unrelated animation is added. Final hosted acceptance remains pending.
