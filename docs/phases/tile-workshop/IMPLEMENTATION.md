...
状态：**用户故事已完成源码评审，待用户确认；尚未开始生产代码重构。**
...
2. **共用编辑器由 Shell 临时编辑会话承载。** 从驾驶台固定水深不启动一次工坊应用、不创建额外最近任务，也不改工坊已有的浏览状态。
...
| 真实开始屏幕 | 拖动、让位、尺寸快捷操作、长按编辑、滚到指定实例 | 替工坊重选内容 |
| 内容提供者 | 声明可固定内容、读取所需摘要、解析查看目的地 | 改来源、启动业务、持久化第二份业务资料 |
| 各应用与领域 | 读数来源、导航/记录/守锚、收藏、AIS 的唯一事实与完整操作 | 为每块磁贴创建一个新 AppId |
...
| S03 从眼前内容固定 | 驾驶台水深详情 → 固定 → 已绑定水深的编辑器 → 添加 → 回同一详情 | 不重选内容、不打开工坊任务、不改来源；航线/地点同理 |
| S04 修改这块尺寸 | 桌面长按水深 → 编辑 → 宽 → 保存 → 原桌面 | 保留 tileId；工坊不展示完整可拖桌面；快捷改尺寸也使用同一提交规则 |
| S05 再次固定同内容 | 任何入口识别已有水深 → 已在开始屏幕 → 编辑这块 / 查看位置 | 不新增、不覆盖样式，不把它说成错误 |
| S06 更换内容 | 编辑水深 → 更换内容 → 同一内容选择器 → 返回草稿 → 保存 | 原 tileId 和位置尽量保留；如航速已有实例，先显示已有项选项并保留水深草稿；明确选择编辑已有项时才按脏草稿规则切会话 |
| S07 固定回家路线 | 已保存路线详情 → 固定 → 预览名称/概要 → 添加 → 原路线详情 | 绑定 stableId，不开始导航；重命名跟随，同名不同 ID 可以并存 |
...
| 当前导航 `CurrentTask(navigation)` | 宽；简洁 / 详细 | **需接入**当前导航角色目的地 | 指向实际活动导航展示，不落到海图上另一条预览路线 |
| 当前守锚 `CurrentTask(anchorWatch)` | 宽；简洁 / 详细 | **需接入**当前值守角色目的地 | 状态、警报、暂停和船位时效常驻；未值守不自动转设置草稿 |
| 当前记录 `CurrentTask(recording)` | 宽；简洁 / 详细 | **需接入**当前记录角色目的地 | 点击时解析会话；暂停仍是同一角色，无记录不自动开始 |
...
AIS 概况是持续能力摘要，不伪装成可开始/结束的会话。上述角色目的地尚未作为独立生产入口存在，必须在这一轮接入所属应用的真实展示状态，不能只登记字符串。内容目录只收有可用渲染和真实点击入口的声明。
...
以下名称为拟议施工类型，不表示已有 API；具体命名可随现有包结构调整，字段语义不可省略。
...
| `InstrumentsExperience.kt:40–43,175–205` | 已有指标详情深链 | 从实际指标详情固定 |
| `MySailingExperience.kt:119–169,204–216`、`PlacesScreen.kt:17–23` | 地点/路线详情、保存回执、路线活动快照回退 | 固定已保存对象；区分收藏删除与活动任务 |
...
| 批次 | 必须进入生产路径的结果 |
| --- | --- |
| 1 独立实例 | 航速/水深从真实详情固定、同屏共存、分别直达、修改/移除独立；同时完成 typed schema、旧迁移顺序、恢复、单提交/失败回执及最小共用编辑器、按绑定订阅。 |
| 2 工坊流程 | 添加/已固定、搜索与分组、同一编辑器、桌面 EditTile、显式定位、取消与冲突；保持三入口来路。 |
...
建议确认的是这组用户故事：**内容优先、同内容单实例、三个入口共用 Shell 临时编辑器、保存回原页、点按磁贴直达查看而不启动业务、布局迁移与可靠落盘先行**。新增状态和冲突处理都是为了让这些故事完整，不扩充工坊的业务职责。

确认后按上表推进生产代码；当前仅完成需求/源码评审和本施工文档。接口名、路由名、schema 版本以落地时当前 HEAD 为准，并回写现有权威契约，不能把本文件的拟议类型宣称已接通。
