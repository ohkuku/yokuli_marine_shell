...
Yokuli 的方向是有统一桌面、磁贴、返回关系、通知与船舶数据的航海 OS。当前生产视觉从 WP8 升级为 **Windows 10 Mobile / MDL2**，以 2015–2016 年微软一手资料为依据，具体来源与共享组件见 [W10M 设计基线](../product/WINDOWS_10_MOBILE_DESIGN.md)。历史 WP8 参考保留，不再覆盖当前视觉指示。通知历史现已通过同包 Binder 进入独立消息进程；Shell 面板仍是应用内覆盖层。R0 将同一 Shell 用作 Android HOME，继续使用 Android 系统服务，不能将默认桌面写成整个系统界面已被替换。
...
**现有：** `MainActivity` 的全屏/系统栏控制用于沉浸展示。系统栏可以由 Android 手势临时出现；这不是替换 SystemUI，更不是屏蔽所有系统入口。R0 不保证首次设置、锁屏、键盘、权限弹窗和外部应用具有 Windows 10 Mobile 视觉风格。
...
页面使用共同的 24sp 标题、20sp 分组/对话框标题、15sp 正文与 12sp 辅助信息；关键海事读数保留更大的数值显示。共享单选、复选、开关、进度、对话框与 CommandBar 统一浅/深色及触摸状态，不再由各应用自造视觉。内容优先保留地图/模型空间；低优先级动作进入真实溢出菜单。字阶、移动端手势、字体授权和平台差异的具体适配见 W10M 基线，控件外观变化不改变业务操作语义。
...
- 通知/快捷入口使用系统目的地导航：同一底层对象只收起并聚焦；打开新目标保留“原页 → 通知中心 → 目标”的访问边界，Back 先退目标内部页，再恢复中心的原滚动/展开状态；Home 放弃这条链。
...
**现有：** Shell 使用自身页面实例和窗口快照呈现内部应用卡片，应用标题位于快照上方，关闭按钮独立保留命中区。向上拖动直接跟手，释放携带速度收敛，重新抓取可取消回弹/关闭；关闭回调只在完成后执行，宿主销毁取消回调。卡片恢复的是内部页面，不是 Android 进程。快照采集只覆盖自身窗口内容，不读取别的应用私有画面。
...
**现有生产实现：** `NotificationClient` 连接同包 `:notifications` 服务，消息仓储保存结构化标题/正文、发布者、对象目标、事件 ID、首次/更新时间、次数、可清除与已读属性；Shell 的 `SystemNotificationStore` 只保留投影与 toast。`MarineNotificationEvents` 从既有守锚/AIS 事件、航行请求账本和运行时待消费反馈发布，面板不是事件产生或业务运行的前提。

通知中心采用固定标题、当前风险摘要、单一滚动内容和底部收起把手，通知历史按发布应用分组，只在组头显示应用身份。四个快捷入口为夜间显示、屏幕常亮、船位来源、船舶连接；前两项是持久偏好开关，后两项为只读摘要导航。扩展入口不占主格，手机安装进入数据中心完整流程。活动守锚、记录、AIS 单独显示任务卡，状态与操作文案分开；暂停守锚必须明确说明影响。

`NotificationShadeState` 按真实高度维护两个锚点，拖动直接更新位置、释放按距离/速度收敛，新手势能接管当前动画。标题和底部把手始终可关闭；消息列表优先滚动，仅末端重新开始的明确拖动可交给面板，惯性碰边不关闭。整页与单条横滑互斥；可见或触摸仍未结束时屏蔽底层输入、焦点和截图捕获。

打开中心不全量已读。稳定展开、约半行可见且持续约 400ms 才按统一政策已读；点击详情也可已读。横滑/全部清除只处理可清除历史，活动任务与警报不在历史里被滑走。空态是“暂无通知记录”，不是安全判断；有限持久化故障显示重试，不递归生成保存失败消息。完整产品、手势、协议和迁移规则见 [通知中心契约](../product/NOTIFICATION_CENTER_CONTRACT.md)。
...
## 9. Windows 10 Mobile 体验在系统化时保持一致

**现有基线：** 内部页面采用 W10M 的 24/20/15/12sp 文字层级、Pivot、即时开关/提交式复选/互斥单选、固定底部 CommandBar、磁贴与平面转场；字号、颜色、间距与触控区域走共同组件。数据中心、设置与磁贴库使用同一 `AppPageTransition`，按原路径深度反向返回，退出页关闭输入且容器不发生尺寸动画。地图、守锚等连续操作内容使用共同地图交互，不额外复制原生地图实例。WP8 参考仅保留历史依据，来源和实际适配以 W10M 设计基线为准。
...
- R0 先保证 Yokuli 内部一致；Android 系统对话框仍呈现平台样式，不能宣称整机已完整复刻 Windows 10 Mobile。
