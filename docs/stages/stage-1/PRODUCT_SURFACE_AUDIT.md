...
| Settings 只讲真实状态 | 外观、语言、地图配置与 About 使用真实 state/build facts；Overview 是索引，Start Screen 是已实现的 Launcher 布局配置，不是额外应用或海事占位功能。 | `PASS_CANDIDATE_REVALIDATED` |
...
| Shell Lab 不进入 Release | app 只用 `debugImplementation(project(":feature:shell-lab"))`；独立 release APK Gate 用 `apkanalyzer` 确认 `ShellLabActivity` 不在 manifest/Dex，且 Chart/Settings classes 存在。 | `PASS_CANDIDATE_REVALIDATED` |
...
`feature:shell-lab` 继续存在于工程源码，供 debug 场景使用；它不是 release catalog contribution，只有 debug dependency，release manifest 也没有 `ShellLabActivity`。
...
Status is `PENDING_HUMAN_REVIEW`. Stage 1 starts exactly from the annotated Stage 0 approval tag and revalidates, rather than inherits, the candidate work first present in `ca84ef9`. The production catalog, Start document, and All Apps contain exactly Chart and Settings. Chart exposes Browse only. Settings contains truthful shell configuration and build facts. Active cockpit, library, and system module sources are absent. Static tests reject Coming Soon and known fabricated marine values. The release APK is assembled and inspected with `apkanalyzer` to prove that Chart and Settings code is present while `ShellLabActivity` is absent. No Stage 2 work is included.
