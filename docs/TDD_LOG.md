...
首次 hosted run 的主构建已绿并上传候选 APK，但 API 36 smoke 的单一 Gradle invocation 把 app-shell class filter 传给 adapter test APK，形成精确 `ClassNotFoundException`。`69cb78b…` 用静态合同先得到 `1/12` Red；`29c758b…` 把两个 instrumentation runner 分开。随后只跑受影响范围：P7 `12/12`、CI topology PASS、adapter `2/2`、app-shell smoke `1/1`，没有因此重跑完整本地 Gate。
