...
> 本文保留 0.4 历史决策。2026-09-17 的 0.5 实现与最新边界以 [完整接口与拓扑](OS_INTERFACE_TOPOLOGY.md) 为准：数据来源归 NMEA、同一应用一块磁贴、声纳调查移除、普通入口打开应用首页，最近任务恢复子页。

日期：2026-09-09。状态：本轮整改的产品与技术决策，补充到 **0.4.0-domains.2**；本版正在运行检查。本文区分目标与已接入代码，**不把模型描述当作 APK 验收结果**。
...
九个业务 App 是本轮目录，另有属于 Shell 的系统工具「磁贴库」。功能扩展先归入明确的用户任务，不能因为存在一组旧代码就新增航海 App。每个业务 App 的首页呈现正在操作的对象：地图、记录、船体、仪表或连接，而不是设置列表。技术详情放在对象详情或诊断中。
...
**系统工具：磁贴库 / Tile Library。** 拥有磁贴预设的预览、固定与移除入口，读取各 App 的真实内容和 Shell 布局；不拥有航程、导航、锚警或连接。它在应用列表有独立入口，属于系统组织能力，不改变九个业务 App 的职责。0.4.0-domains.2 已接入这一目录与内容入口，运行验收单独记录。
...
flowchart TB
  ...
  Shell --> Apps
  Shell --> Tiles[系统工具磁贴库：预设、固定与布局入口]
...
- 每条连接和输出的用户运行意图分别处理。恢复航程不能重开用户主动断开的连接或已停止的输出；保存地址和路由不代表开始运行。当前新连接目录持久化配置，进程重建后默认停止；完整的按意图自动恢复不能视为已完成。
...
系统工具磁贴库的故事是：预览真实内容 → 固定所需的专用磁贴 → 在 Start 调整尺寸与位置 → 从磁贴恢复对应 App → 调整内容轮换偏好或移除磁贴。海图画面、导航、速度、航向、水深、风、锚距、记录和 NMEA 流量各有预设；同一 App 的不同预设可以并存。当前不提供任意复制同一个预设的编辑器。
...
没有现存任务时，使用对应 App 图标、名称与原 Shell 启动转场。已有任务从普通入口或磁贴打开时，激活该任务并保留当前子页；明确的对象深链仍打开指定对象。Shell 按任务和入口保存已接入的页面状态，NMEA 连接详情等使用可保存状态参与恢复。此处的冷/热指 Shell 任务生命周期，不承诺 Android 进程被杀后还原全部临时界面。
...
| 保留 | 重新组织或替换 |
| --- | --- |
| 原 Shell 引擎、Compose 组件、布局存储、AppPreference 契约和 WP8 参考 | 九业务 App 与系统工具目录、旧入口映射、实际任务预览和磁贴预设；不复制 Shell 样式另写一套。 |
