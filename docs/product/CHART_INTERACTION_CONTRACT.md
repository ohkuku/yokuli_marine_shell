...
1. 图库的“底图”保留文件夹 / MBTiles 显示能力；“航行数据”管理未加密 S-57 `.000`、连续更新、ZIP / 目录，以及 GeoPackage `.gpkg` 开放矢量包。先进入资料包，再按图幅、对象分类与名称搜索真实对象，详情展示深度依据、来源和属性，可发起只读地图预览。导入和浏览不自动更换当前资料组合。
2. 海图来源面板分别选择背景与多个航行数据集。切换卫星或栅格背景不清空所选数据；没有数据时可继续看图、手工画线，但不能把底图颜色当成深度。
3. 海图“航线规划”选择起终点，或选择已有航线/草稿；“检查航线”给出问题与沿程证据，“自动规划”只在资料门槛通过后生成直达或绕行候选。全线和指定航段都保留原几何，方案在海图并列比较后才显式采用。
...
8. 规划页直接提供“选择航行数据”“管理 / 导入数据”和“手动绘线 / 继续手动编辑”。到图库返回后保留同一次规划输入；手动入口不启动自动规划，不覆盖现有草稿。没有草稿时可使用已明确选择的起终点，保存回执成功后再回海图编辑。资料缺失、过期或输入已变时，旧候选仍可作为历史结果查看，不能直接采用或替换当前导航。

### 真实数据链与边界

`ChartDataService` → `LocalChartDataService` → Android 文档授权 / 临时复制 → S-57 连续更新解析或 GeoPackage 完整包解析 → 共用对象索引 → 不可变 SQLite + RTree 版本 → 原子安装指针 → 当前明确选择的快照租约 → 图库对象浏览 / 只读预览 / 地图 / 航线检查与规划。
