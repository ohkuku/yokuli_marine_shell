...
| `MapSource` | `Online`, `Satellite`, `CustomLayer(layerId)` | 在线、卫星、自定义文件夹图层；不是多组互相冲突的开关 |
...
比例尺使用整数 `1/2/5 × 10^n`，根据真实投影距离改变线段长度。近景用米，远景按全局单位用海里或千米；它不会把固定像素宽度算成任意小数再当作刻度。

## 海图用户故事

1. 拖动地图：关闭跟随，准星显示；坐标紧贴底部操作栏。显示准星或坐标时，原生地图的尺寸保持不变。
2. 点中收藏：在海图上高亮该坐标，显示名称、格式化坐标和明确的详情入口。只有再点“地点详情”才打开“我的航行”的对应对象。
3. “更多 → 我的航行”：请求 `places` 根入口。Shell 负责进入应用根页；最近任务卡片仍恢复原来的子页面。
4. 标记：立即保存当前位置，在本地图显示刚保存的地点预览；不把操作偷偷变成应用跳转。
...
9. 海图来源：选择在线、卫星或一个命名图层；自定义海图无覆盖时保留内置 Natural Earth 全球离线背景。声纳调查入口已从海图移除。
