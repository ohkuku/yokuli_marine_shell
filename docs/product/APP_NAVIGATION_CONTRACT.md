...
当前目标卡可直接前往下一点；已到目标附近时不再绕进管理弹窗。尚未确认到达时跳点、确认终点并结束均需明确确认。推进动作携带用户所见的 sessionId、revision 和 requestId。运行时串行核对并保存回执，旧面板不能在目标或路线版本已变化后再多跳一次。
...
`MarineSystem.navigation: NavigationSessionService` 是唯一业务所有者，standalone/ROM 装配当前都使用 `LocalNavigationSessionService`。`OsStore.navigationState` 只是显示投影；`navigationRoute`、`activeRouteId`、`routeLeg` 是只读兼容访问。地图预览、准星、显示哪条收藏路线仍属于本次 UI 访问，不能通过恢复页面快照回滚导航。

- `NavigationRouteSnapshot` 冻结路线 ID、内容修订、航点 ID 和几何。修改/删除收藏不改变执行副本；`REPLAN` 是显式命令。手动选目标保留当时船位为接入段起点，推进后才使用上一航点，不从被跳过的点计算横偏。
- 所有命令使用 `NavigationCommand(requestId, expectedSessionId, expectedRevision, action, …)`。保存路线/目标/设置与最近 32 条终态回执在同一 AtomicFile 提交；成功反馈发生在落盘之后。重复请求重放原回执，冲突重新打开操作，写入失败可以用同一请求重试。进程内已接受命令不因页面关闭而取消。
- 本地引导使用 WGS84 测地线距离和方位。正横偏表示规划线右侧；船位在航段延长线上时不给出伪装成横偏的端点距离。仪表、导航磁贴、海图和立体方向都消费同一会话目标。旧历史不回写，新导航读数按会话修订分段。
- 默认手动推进。用户可以在本次导航内开启自动推进：必须先从到达范围外接近，船位新鲜、已知精度满足限制、连续三个独立观测持续至少三秒，并在半径内或符合通过线/走廊条件；每次只推进一点，切源、修订改变、失去精度或时效会清除候选。终点进入 ARRIVED，不结束航行记录。
- ETA 可明确采用已保存计划航速，或新鲜 SOG/COG 在目标方向的有效分量。低速、背离目标、关键输入过期时不输出当前进度 ETA；计划估计注明依据。观测未更新不把同一距离的预计到达时刻不断往后移动。Heading 与 COG 保持独立，导航不参与传感器选源。
- `EXTERNAL_NMEA` 必须由用户明确选择连接、peer、talker 对应的目标来源。外部报文不能覆盖 LOCAL 会话；航点推进由外部设备负责。RMB/BWC/BWR 仅在同一物理来源、同一观测报文中实际提供经纬度时才显示目标位置；只有距离/方位时不推算地图坐标。APB 使用船到目标方位 11/12 字段并要求真北参考，BOD 的固定航段方位不能冒充船到目标方位。原始 NMEA XTE 的操舵方向转换成统一规划线侧向语义。

完整路线折线和用户目的地分别保存：`Route.navigationTargetIndices` 为完整几何中的业务目标索引，旧路线缺失该字段时每点仍是业务目标。自动规划圆弧细分点只负责路线形状，收藏详情、目标列表、地图标号及导航推进只展示/确认业务目标。运行时 `targetIndex` 是业务目标的几何索引，`geometryIndex` 是当前沿线路径段；形状段跟随不产生业务 ADVANCE 或新的会话修订。`steeringBearingTrueDegrees` / `steeringPosition` 用于沿线方向，`bearingTrueDegrees` / `targetPosition` 仍指真实目的地，避免把绕行中的直达目的地方向冒充安全转向。规划、重规划和 GPX 导入导出都保留映射，无法保留用户指定中间目标的候选不予采用。

完整恢复和后台限制见 [生命周期与恢复](../os/03-LIFECYCLE-AND-RECOVERY.md#导航执行运行时2026-09-25)。当前端口已真实接入，仍采用进程内实现，不表示独立 Marine Core Binder、自动驾驶输出或设备级后台可靠性已经完成。
...
数据中心、设置的局部层级共用 `AppPageTransition`：只消费原 path/section/owner，不另建返回栈。内容按页面键复用，草稿和滚动继续使用各自 `SaveableStateHolder`；深度减小采用反向位移。退出页通过 `LocalInternalAppInputEnabled=false` 注销局部 Back，Initial pointer pass 拦截触摸，并清除交互语义。容器尺寸固定，无 SizeTransform；不能将持有原生地图的两页无条件同时挂载。
...
| 设置 → 磁贴工坊 | 旧地址规范化为真实工坊应用；关联访问返回原设置页，不把工坊嵌套成设置子页面。 |
...
## 磁贴内容与临时编辑（2026-09-25）

工坊“添加/已固定”、桌面“编辑内容”、应用“固定到开始屏幕”共同调用 `WpShellRuntime.tileWorkshop`，由 Shell 顶层唯一 `TileEditorHost` 承载。编辑不创建 AppId 或最近任务，不改变底下 App 的页面键、滚动、选中内容与访问链。通知覆盖编辑器时停止其输入、Back 与预览观察；普通返回先退出内容选择器，再按脏草稿决定关闭/确认。
