...
if [[ "$release_tag" =~ ^v(0|[1-9][0-9]*)\.(0|[1-9][0-9]*)\.(0|[1-9][0-9]*)-alpha\.([1-9][0-9]*)$ ]]; then
  ...
  prerelease_number="${BASH_REMATCH[4]}"; release_channel='alpha'; stage_code=$((1000 + 10#$prerelease_number))
elif [[ "$release_tag" =~ ^v(0|[1-9][0-9]*)\.(0|[1-9][0-9]*)\.(0|[1-9][0-9]*)-beta\.([1-9][0-9]*)$ ]]; then
  major="${BASH_REMATCH[1]}"; minor="${BASH_REMATCH[2]}"; patch="${BASH_REMATCH[3]}"
  prerelease_number="${BASH_REMATCH[4]}"; release_channel='beta'; stage_code=$((5000 + 10#$prerelease_number))
elif [[ "$release_tag" =~ ^v(0|[1-9][0-9]*)\.(0|[1-9][0-9]*)\.(0|[1-9][0-9]*)$ ]]; then
  major="${BASH_REMATCH[1]}"; minor="${BASH_REMATCH[2]}"; patch="${BASH_REMATCH[3]}"
  prerelease_number='0'; release_channel='stable'; stage_code=9000
else
  die 'Tag must be vX.Y.Z, vX.Y.Z-alpha.N, or vX.Y.Z-beta.N without leading zeroes.'
fi

(( 10#$major <= 20 )) || die 'Major version must be 20 or lower.'
...
if [[ -n "${GITHUB_OUTPUT:-}" ]]; then emit_metadata >> "$GITHUB_OUTPUT"; else emit_metadata; fi
