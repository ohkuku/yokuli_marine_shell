...
probe_class='com.yokuli.marine.shell.NmeaP6ProcessRestartProbeTest'
...
cd "$repo_root"
mkdir -p build
process_log='build/ci-nmea-sources-process-restore.log'
: > "$process_log"
./gradlew --no-daemon \
...
run_probe() {
  local method="$1"
  local output
  output="$(mktemp -t yokuli-nmea-process.XXXXXX)"
  ...
  if grep -Eq 'FAILURES!!!|INSTRUMENTATION_FAILED|Process crashed' "$output" || ! grep -Fq 'OK (1 test)' "$output"; then
    printf 'NMEA process-restart probe failed: %s\n' "$method" >&2
    rm -f "$output"
    return 1
  fi
  rm -f "$output"
...
printf 'NMEA Sources P6 external process-restart probe\n' | tee -a "$process_log"
run_probe seedNmeaStateBeforeExternalProcessRestart
"$adb_bin" shell am force-stop com.yokuli.marine
run_probe verifyPolicySurvivesButLiveValuesDoNot
[[ "$("$adb_bin" shell pm clear "$target_package" | tr -d '\r')" == 'Success' ]]
printf 'NMEA Sources P6 policy persisted and live values stayed process-local\n' | tee -a "$process_log"
