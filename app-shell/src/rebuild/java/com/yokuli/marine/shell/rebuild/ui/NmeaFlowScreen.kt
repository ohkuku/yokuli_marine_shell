    ...
    Column(Modifier.fillMaxSize()){
        ...
        pageStates.SaveableStateProvider(pageKey) { when{
            ...
            current!=null->Pivot(listOf(os.t("实况","live"),os.t("语句","sentences"),os.t("路由","routing"))){page->
                when(page){
                    0->PageBody{
                        Label(connectionText(os,current),24,LocalMetro.current.accentText)
                        ...
                        Row(Modifier.fillMaxWidth(),horizontalArrangement=Arrangement.spacedBy(24.dp)){
                            Column(Modifier.weight(1f)){Label(current.diagnostics.validSentences.toString(),24);Label(os.t("接收有效语句","valid received"),12,LocalMetro.current.muted)}
                            Column(Modifier.weight(1f)){Label(current.writtenSentences.toString(),24);Label(os.t("实际写出","actually written"),12,LocalMetro.current.muted)}
                        }
                        ...
                        current.error?.let{Label(it,16,LocalMetro.current.accentText)}
                        ...
                        if(!current.requested&&current.spec.send&&current.spec.feed==NmeaFeed.PHONE)Label(os.t("先编辑这条连接，将旧版手机专用输出改为数据中心的读数。","Edit this connection to replace legacy phone-only output with Data Center readings."),15,LocalMetro.current.muted)
                    ...
                    1->PageBody{
                        ...
                        if(live.isEmpty())Label(os.t("尚无语句","No sentences yet"),15,LocalMetro.current.muted)
                    }
                    else->PageBody{
                        Label(if(current.spec.receive)os.t("接收 → 数据中心","receive → data center")else os.t("不接收数据","input disabled"),20)
                        Label(if(current.spec.send)os.t("输出：","output: ")+feedText(os,current.spec.feed)else os.t("输出已关闭","output disabled"),20)
                ...
                if(selected!=null)Label(os.t("这条连接正在读取，或已被移除。返回会保留原来的数据详情。","This connection is loading or has been removed. Back keeps your original data detail."),15,LocalMetro.current.muted)
                AppSection(os.t("船上的连接","connections aboard"))
                Label(os.t("连接设备、接收与发送。收到的读数汇入数据中心，在那里统一选择每项数据的来源。","Connect devices, receive and transmit. Received measurements arrive in Data Center, where you choose their sources for all apps."),15,LocalMetro.current.muted)
                if(connections.isEmpty())Label(os.t("添加 GPS、风仪或网关。每条连接可以接收、发送，或双向运行。","Add a GPS, wind instrument or gateway. Each connection can receive, send, or do both."),15,LocalMetro.current.muted)
        ...
        state.connectionAttempt.message.takeIf{state.connectionAttempt.state==com.yokuli.anchorwatch.ConnectionAttemptState.FAILED&&it.isNotBlank()}?.let{Label(it,14,LocalMetro.current.accentText,Modifier.padding(start=LocalShellHorizontalInsets.current.pageStart,end=LocalShellHorizontalInsets.current.pageEnd,top=8.dp,bottom=8.dp))}
    }
}
...
@Composable private fun ConnectionEditor(os:OsStore,initial:NmeaConnectionSpec?,all:List<NmeaConnectionSpec>,save:(NmeaConnectionSpec)->Unit){
    ...
    PageBody{
        ...
        AppCheckRow(os.t("接收数据","receive data"),rx){rx=!rx}
        AppCheckRow(os.t("发送数据","send data"),tx){tx=!tx}
        ...
        if(advanced){
            if(rx){AppCheckRow(os.t("忽略传输中损坏的数据","ignore damaged data"),checksum){checksum=!checksum};Label(os.t("设备附带的校验码可以发现传输错误。默认开启；只有老设备不附带校验码时才关闭。","The device's error-checking code detects damaged packets. Leave this on unless an older device omits the code."),15,LocalMetro.current.muted)}
