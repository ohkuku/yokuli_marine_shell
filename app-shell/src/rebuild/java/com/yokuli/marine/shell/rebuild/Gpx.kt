...
object Gpx {
        ...
        while(parser.eventType!=XmlPullParser.END_DOCUMENT) {
            if(parser.eventType==XmlPullParser.START_TAG) when(parser.name) {
                ...
                "wpt","rtept","trkpt"->{
                    ...
                    if(parser.name=="wpt") {mark=p;markName="";note=""} else points.add(p)
                }
                "name"->{val value=parser.nextText().take(100);if(mark!=null) markName=value else if(inRoute && points.isEmpty()) routeName=value}
                "desc"->{val value=parser.nextText().take(2000);if(mark!=null) note=value}
            }
            if(parser.eventType==XmlPullParser.END_TAG) when(parser.name) {
                "wpt"->{mark?.let { places+=Place(name=markName.ifBlank {"GPX ${places.size+1}"},point=it,note=note) };mark=null}
    ...
    fun write(output:OutputStream,places:List<Place>,routes:List<Route>) {
        ...
        fun point(tag:String,p:GeoPoint,name:String?=null,note:String?=null) {
            x.startTag(null,tag).attribute(null,"lat",p.lat.toString()).attribute(null,"lon",p.lon.toString())
            name?.let {element("name",it)};note?.takeIf {it.isNotBlank()}?.let {element("desc",it)};x.endTag(null,tag)
        }
        places.forEach {point("wpt",it.point,it.name,it.note)}
