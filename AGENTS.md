...
- 图册统一配置海图背景和启用的航行数据：MBTiles / 海图文件夹只供显示，S-57 / GeoPackage 数据目录供查询与规划。海图来源弹层只给图册入口，规划页不得再选数据集或把背景当计算依据；保留原来源持久化键，不另建每条航线的来源设置。航线编辑只使用 `OsStore.draftRoute` 及其目标映射，`RouteDraftControls` 和检查建议面板共用该草稿；不能恢复独立的起终点规划器。格式、快照租约、单一用户流程和数据门槛见 [海图契约](docs/product/CHART_INTERACTION_CONTRACT.md#数据图册与自动规划2026-09-25)。开放包仍按 [GeoPackage profile](docs/GEOPACKAGE_CHART_PROFILE.md) 编辑原文件并整包更新，原始资料不被 App 改写。导航由 `MarineSystem.navigation` 单写；保存、接受建议或离开编辑不隐式开始或替换导航。
