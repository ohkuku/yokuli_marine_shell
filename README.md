...
以 Windows Phone 8/10 的排版、横滑、磁贴和虚拟键组织的 Android 航海应用平台。当前版本 **0.5.0-experience.3**。本轮集中修复地图定位、应用返回、全局记录与数据分享，并重做锚警、仪表、磁贴和通知。

完整结构见 [26 项反馈、全部应用接口与数据拓扑](docs/product/OS_INTERFACE_TOPOLOGY.md)，声明位置见 [API 索引](docs/product/API_INDEX.md)。实际验证结果与边界见 [本轮交付记录](docs/experience/EXPERIENCE_3_DELIVERY.md)。
...
| 海图库 | 文件夹、命名图层、扫描、改名、文件启停与优先级、移除/恢复、解除关联。 |
| 航行日志 | 当前航程、时刻与笔记、暂停/继续/保存、历史回放/报告/导出、在海图预览。 |
| 锚警 | 下锚、选点和半径、值守、近期渐隐轨迹、累计停留区域、起锚与收藏。 |
| 我的航行 | 坐标、航线、收藏锚地、集合、GPX、预览与导航。 |
| 仪表 | 真实罗盘、风向、姿态、量表与趋势；自选仪表、拖动和上下排序。 |
| NMEA 输入及输出 | 多连接、逐字段来源、报文与流量、按能力分享或转发。 |
| 本机 NMEA 客户端 | 本机监听服务、实际连接客户端、分享能力与来源选择。 |
| 设置 | 船名与船舶资料、语言、单位、坐标格式、主题色、文字大小、常亮、声音、权限、资料备份。 |

磁贴库是 Shell 工具：先选择应用，再横滑预览样式和大小。同一应用只保留一块磁贴，切换样式保留位置；实时读数与应用使用同一来源。海图图像快照显示捕获时间，船位和速度独立实时更新。

## 一套系统

- 普通应用入口打开首页；最近任务恢复上次子页。任务卡使用真实窗口截图，冷启动才显示应用启动画面。根页不再显示“返回 OS”，首页键负责回桌面。
...
- 船位和各读数来源集中在 NMEA 的“数据来源”。选中来源失效不会偷偷切换。GPS 短时未更新只显示时效，不反复遮挡操作。
- 本机分享与主动发送共用发布规则；按能力筛选实际报文，保留来源与连接代次，禁止给相同 IP 回送其输入数据。
- 轻提示先出现在系统栏，底部右键（原搜索键）打开通知中心，中间 Home 始终返回开始屏幕；通知中心并保留应用、正文、时间和详情；顶部下滑完全交还 Android 系统。阅读通知不解除业务警报。
- 统一字体、方形单选、矩形输入框、横滑和细节动效。全局单位、坐标格式、文字大小在应用和磁贴同步。全屏控件按圆角横向避让。
...
Version **0.5.0-experience.3** unifies app navigation, global voyage recording, notifications and data publication. Chart annotations render natively with the map camera; anchor watch emphasizes the map and real swing history. Instruments use meaningful live graphics and a reorderable personal layout. Tile Library presents one app at a time, with one configurable tile per app. Local NMEA and external outputs share actual capability filtering and source-aware IP echo prevention. Sonar survey UI, acquisition and automatic restoration are removed while historical records are retained.
