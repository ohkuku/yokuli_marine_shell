...
class VesselDataHub @Inject constructor(navigation:NavigationRepository,depth:LiveDepthRepository,wind:LiveWindRepository,phoneHeading:PhoneHeadingRepository,positions:VesselPositionRepository,settings:VesselSettingsRepository,attitude:PhoneVesselAttitudeRepository,pressure:PhonePressureRepository,nmeaFields:NmeaFieldRepository,private val sourceRegistry:VesselSourceRegistry,mountCalibration:VesselMountCalibrationRepository,private val pressureHistory:PressureHistoryRepository){
    ...
    @Synchronized private fun publish(now:Long){
        val effectivePositionPreference=tripPositionPreference?:when(shellPositionSource){GpsDataSource.NMEA->VesselSourcePreference.BOAT;GpsDataSource.SYSTEM,GpsDataSource.DEMO->VesselSourcePreference.PHONE;GpsDataSource.NONE->VesselSourcePreference.DERIVED}
        ...
        val selectedPosition=if(shellPositionSource==GpsDataSource.NONE&&tripPositionPreference==null)VesselObservation<VesselPosition>() else selectionObservation(positionSelection)?:VesselSourceSelector.select(effectivePositionPreference,classify(boatPosition,now,3_000,30_000),classify(phonePosition,now,3_000,10_000))
