...
class TripExportManager @Inject constructor(
    ...
    private suspend fun writeTripSamples(target:File,id:Long)=target.bufferedWriter().use{w->
        w.appendLine("timestamp_utc,timestamp_epoch_ms,latitude,longitude,position_source,position_source_id,position_quality,position_age_ms,sog_knots,sog_age_ms,cog_true_deg,cog_age_ms,heading_true_deg,heading_source,heading_source_id,heading_reference,heading_age_ms,depth_m,depth_source,depth_source_id,depth_age_ms,stw_knots,stw_source,stw_source_id,stw_age_ms,true_wind_knots,true_wind_speed_source_id,true_wind_speed_age_ms,true_wind_direction_deg,true_wind_direction_source_id,true_wind_direction_age_ms,true_wind_angle_deg,true_wind_angle_source_id,true_wind_angle_age_ms,true_wind_provenance,true_wind_reference,apparent_wind_knots,apparent_wind_speed_source_id,apparent_wind_speed_age_ms,apparent_wind_angle_deg,apparent_wind_angle_source_id,apparent_wind_angle_age_ms,wind_source,legacy_wind_age_ms,heel_deg,pitch_deg,roll_rate_deg_s,pitch_rate_deg_s,yaw_rate_deg_s,motion_score,roll_period_s,roll_period_confidence,attitude_age_ms,attitude_quality,attitude_mount_suspect,pressure_hpa,pressure_age_ms,ukc_m,publication_ownership_state,source_flags")
    ...
    private suspend fun writeAnchorTelemetry(target:File,id:Long)=target.bufferedWriter().use{w->w.appendLine("timestamp_utc,timestamp_epoch_ms,depth_m,depth_age_ms,apparent_wind_knots,apparent_wind_speed_age_ms,apparent_wind_angle_deg,apparent_wind_angle_age_ms,true_wind_knots,true_wind_speed_age_ms,true_wind_direction_deg,true_wind_direction_age_ms,true_wind_angle_deg,true_wind_angle_age_ms,true_wind_provenance,true_wind_reference,heading_true_deg,heading_source_id,heading_age_ms,heel_deg,pitch_deg,roll_rate_deg_s,pitch_rate_deg_s,yaw_rate_deg_s,motion_score,roll_period_s,roll_period_confidence,attitude_quality,attitude_mount_suspect,pressure_hpa");pageAnchorTelemetry(id){s->w.row(Instant.ofEpochMilli(s.timestamp),s.timestamp,s.depthMeters,s.depthAgeMillis,s.apparentWindSpeedKnots,s.apparentWindSpeedAgeMillis,s.apparentWindAngleDegrees,s.apparentWindAngleAgeMillis,s.trueWindSpeedKnots,s.trueWindSpeedAgeMillis,s.trueWindDirectionDegrees,s.trueWindDirectionAgeMillis,s.trueWindAngleDegrees,s.trueWindAngleAgeMillis,s.trueWindProvenance,s.trueWindReference,s.headingTrueDegrees,s.headingSourceId,s.headingAgeMillis,s.heelDegrees,s.pitchDegrees,s.rollRateDegPerSec,s.pitchRateDegPerSec,s.yawRateDegPerSec,s.motionScore,s.rollPeriodSeconds,s.rollPeriodConfidence,s.attitudeQuality,s.attitudeMountSuspect,s.pressureHpa)}}
...
- `*_source_id` is the stable producer identity (transport profile/generation plus sentence or phone sensor); source labels are presentation-only.
- `true_wind_provenance` and `true_wind_reference` distinguish external NMEA, water-referenced derivation and ground-referenced fallback. Derived wind is not an Anchor safety input.
- Source-timeline events in `report.json` explain canonical source changes and Phone takeover without rewriting prior samples.
