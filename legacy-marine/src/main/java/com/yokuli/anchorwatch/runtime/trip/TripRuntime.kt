...
class TripRuntime @Inject constructor(
    ...
    private fun sample(session:TripSessionEntity,s:VesselDataSnapshot,wall:Long,elapsed:Long,recordingSequence:Long):TripSampleEntity{
        ...
        return TripSampleEntity(
            ...
            heelDegrees=attitude.value?.heelDegrees,pitchDegrees=attitude.value?.pitchDegrees,rollRateDegPerSec=attitude.value?.rollRateDegreesPerSecond?.takeIf{it.isFinite()},pitchRateDegPerSec=attitude.value?.pitchRateDegreesPerSecond?.takeIf{it.isFinite()},yawRateDegPerSec=attitude.value?.yawRateDegreesPerSecond?.takeIf{it.isFinite()},motionScore=motion.value?.score,rollPeriodSeconds=motion.value?.dominantRollPeriodSeconds,rollPeriodConfidence=motion.value?.rollPeriodConfidence?.name,attitudeAgeMillis=age(attitude,elapsed),attitudeQuality=attitude.quality.name,attitudeMountSuspect=attitude.provenance=="PHONE_MOVED_OR_MOUNT_SUSPECT",
