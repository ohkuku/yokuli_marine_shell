...
private enum class SettingsDestination{ROOT,ALARM,VESSEL,PHONE_SENSORS,DEPTH_SOUNDER,MAP_DEPTH,BACKGROUND,DATA_BACKUP,STORAGE_SUPPORT,ABOUT,FEEDBACK}
...
@Composable private fun SettingsRoot(state:MainUiState,open:(SettingsDestination)->Unit,language:()->Unit,support:(String)->Unit){
 LazyColumn(Modifier.fillMaxSize().padding(horizontal=18.dp).testTag("settings_list"),contentPadding=PaddingValues(vertical=18.dp)){
  ...
  item{SettingsSection(tr("MAP & CHARTS","地图与海图"));SettingsRow(Icons.Default.Layers,tr("Map data","地图数据"),tr("Offline MBTiles · personal sonar","离线 MBTiles · 个人声呐"),"settings_map_depth"){open(SettingsDestination.MAP_DEPTH)}}
  item{SettingsSection(tr("DEVICE & DATA","设备与数据"));SettingsRow(Icons.Default.BatterySaver,tr("Background reliability","后台可靠性"),tr("Permissions, power and Wi-Fi","权限、电源与 Wi-Fi"),"settings_background"){open(SettingsDestination.BACKGROUND)};SettingsRow(Icons.Default.Backup,tr("Data & backup","数据与备份"),tr("Export or replace from a Boat Watch backup","导出或从 Boat Watch 备份替换恢复"),"settings_data_backup"){open(SettingsDestination.DATA_BACKUP)};SettingsRow(Icons.Default.Storage,tr("Storage & support","存储与支持"),tr("Health, incident log and diagnostics","健康、事件日志与诊断包"),"settings_storage_support"){open(SettingsDestination.STORAGE_SUPPORT)};if(!LocalMarineEmbeddedRoot.current)SettingsRow(Icons.Default.Language,tr("Language","语言"),state.settings.appLanguage.nativeName,"settings_language",language)}
  item(key="settings_about"){SettingsSection(tr("ABOUT","关于"));SettingsRow(Icons.Default.Info,tr("About & support","关于与支持"),tr("Developed aboard SV Yokuli","开发于 SV Yokuli 船上"),"settings_about"){open(SettingsDestination.ABOUT)};if(ProductBrand.contactEmail.isNotBlank())SettingsRow(Icons.Default.Feedback,tr("Feedback & feature requests","反馈与功能建议"),tr("Email kuku directly","直接给 kuku 发邮件"),"settings_feedback"){open(SettingsDestination.FEEDBACK)}}
 }
}

@Composable private fun SettingsSubPage(destination:SettingsDestination,state:MainUiState,vm:MainViewModel,back:()->Unit){
 val title=when(destination){SettingsDestination.ALARM->tr("Alarm & notifications","报警与通知");SettingsDestination.VESSEL->tr("Vessel profile","船舶资料");SettingsDestination.PHONE_SENSORS->tr("Phone vessel sensors","手机船舶传感器");SettingsDestination.DEPTH_SOUNDER->tr("Depth sounder","测深仪");SettingsDestination.MAP_DEPTH->tr("Map & depth","地图与水深");SettingsDestination.BACKGROUND->tr("Background reliability","后台可靠性");SettingsDestination.DATA_BACKUP->tr("Data & backup","数据与备份");SettingsDestination.STORAGE_SUPPORT->tr("Storage & support","存储与支持");else->""}
 MarineSubpageTitle(title)
 Column(Modifier.fillMaxSize()){
  if(!LocalMarineEmbeddedRoot.current)MarinePageTitle(title,back)
  LazyColumn(Modifier.fillMaxSize().padding(horizontal=16.dp),contentPadding=PaddingValues(bottom=24.dp),verticalArrangement=Arrangement.spacedBy(12.dp)){
   item{when(destination){SettingsDestination.ALARM->AlarmSettingsPage(state,vm);SettingsDestination.VESSEL->VesselProfileCard(state,vm);SettingsDestination.PHONE_SENSORS->PhoneVesselSensorsCard(state,vm);SettingsDestination.DEPTH_SOUNDER->DepthSounderPage(state,vm);SettingsDestination.MAP_DEPTH->MapDepthSettingsPage(state,vm);SettingsDestination.BACKGROUND->BackgroundReliabilityCard(state,vm);SettingsDestination.DATA_BACKUP->DataBackupPage(state,vm);SettingsDestination.STORAGE_SUPPORT->StorageSupportPage(state,vm);else->{}}}
  }
 }
}
...
@Composable private fun MapDepthSettingsPage(state:MainUiState,vm:MainViewModel){
 if(com.yokuli.anchorwatch.map.SharedChartLayers.managedByShell){
  Column(verticalArrangement=Arrangement.spacedBy(16.dp)){
   Text(tr("All maps use the folders and layers in Chart Library.","所有地图共用海图库中的文件夹与图层。"),style=MaterialTheme.typography.bodyLarge)
   Button({com.yokuli.anchorwatch.map.SharedChartLayers.openLibrary()},Modifier.fillMaxWidth()){Text(tr("Open chart library","打开海图库"))}
   Text(tr("Rename layers, set overlap priority and rescan folders there. Personal sonar and regional depth overlays remain available on the watch map.","在海图库中重命名图层、设置覆盖优先级、重新扫描文件夹。监控地图仍可开启个人水深与区域水深叠加。"),color=MaterialTheme.colorScheme.onSurfaceVariant)
  }
  return
 }
...
@Composable internal fun DataOutputSettingsPage(state:MainUiState,vm:MainViewModel){
 ...
 BackHandler(product!=null){product=null}
 when(product){
...
@Composable private fun PhoneAppBoatOutputPage(state:MainUiState,vm:MainViewModel,back:()->Unit){
 MarineSubpageTitle(tr("send phone data","发送手机数据"))
 val automaticOutput=NmeaOutputEndpointPolicy.automatic(state.outputSettings,state.settings.profile)
 val inputSocketOpen=state.settings.profile.protocol==com.yokuli.anchorwatch.data.nmea.Protocol.TCP&&state.connection in setOf(com.yokuli.anchorwatch.domain.model.NmeaConnectionState.CONNECTED,com.yokuli.anchorwatch.domain.model.NmeaConnectionState.CONNECTED_NO_DATA,com.yokuli.anchorwatch.domain.model.NmeaConnectionState.CONNECTED_NO_FIX,com.yokuli.anchorwatch.domain.model.NmeaConnectionState.STALE)
 val outputReady=automaticOutput.transportConfigured&&when(automaticOutput.transportMode){NmeaOutputTransportMode.SAME_AS_INPUT_CONNECTION->NmeaOutputEndpointPolicy.isValid(automaticOutput,state.settings.profile)&&inputSocketOpen;NmeaOutputTransportMode.TCP_SERVER->false;NmeaOutputTransportMode.DEDICATED_TCP->automaticOutput.outputHost.isNotBlank()&&automaticOutput.outputPort in 1..65535&&!NmeaOutputEndpointPolicy.opensSecondTransportOnInputEndpoint(automaticOutput,state.settings.profile);NmeaOutputTransportMode.UDP_UNICAST,NmeaOutputTransportMode.UDP_BROADCAST->automaticOutput.outputHost.isNotBlank()&&automaticOutput.outputPort in 1..65535}
 ...
 var routeMode by remember(automaticOutput.transportMode){mutableStateOf(automaticOutput.transportMode.takeUnless{it==NmeaOutputTransportMode.TCP_SERVER}?:NmeaOutputTransportMode.SAME_AS_INPUT_CONNECTION)}
 ...
 LazyColumn(Modifier.fillMaxSize().testTag("nmea_output_list").padding(16.dp),verticalArrangement=Arrangement.spacedBy(12.dp)){
  ...
  item{Card{Column(Modifier.padding(16.dp),verticalArrangement=Arrangement.spacedBy(8.dp)){
   ...
   Text(tr("Selected valid values calculated by this phone or Boat Watch are published continuously at a stable 1 Hz. Boat instruments decide which source to display; Boat data received by the App is never copied back. Byte-identical returned App frames are quarantined.","本手机或 Boat Watch 计算出的已选有效值会以稳定的 1 Hz 持续发布。由船载仪表决定显示哪个来源；App 接收到的船载数据绝不会被复制回去。字节完全相同的 App 返回帧会被隔离。"),style=MaterialTheme.typography.bodySmall,color=MaterialTheme.colorScheme.onSurfaceVariant)
   ...
   Surface(color=if(phoneFrameReady)MaterialTheme.colorScheme.secondaryContainer else MaterialTheme.colorScheme.surfaceVariant,shape=MaterialTheme.shapes.small){Text(if(phoneFrameReady)tr("Phone vessel heading is aligned and may publish HDT. If the phone rotates relative to the vessel, realign it here. GNSS and pressure are independent; ROT, heel and pitch require a confirmed Trip attitude segment.","手机船首向已对齐，可以发布 HDT。若手机相对船体发生转动，请在这里重新对齐。GNSS 与气压独立工作；转向率、横倾和纵倾需要 Trip Watch 中确认有效的姿态段。")else tr("Output may start, but Heading will stay WAITING_CALIBRATION until bow alignment is completed. Position and pressure remain independent; Trip mounting is required only for ROT, heel and pitch.","可以启动输出，但在完成船艏方向对齐前，船首向会保持“等待校准”。船位和气压不受影响；只有转向率、横倾和纵倾需要 Trip Watch 的固定安装确认。"),Modifier.fillMaxWidth().padding(10.dp),style=MaterialTheme.typography.bodySmall)}
   Text(tr("What will be shared","将分享的数据"),style=MaterialTheme.typography.labelLarge,fontWeight=FontWeight.SemiBold)
   Text(tr("Presets change only the stream switches below; they never start transmission.","预设只会调整下方的数据流开关，不会自动开始发送。"),style=MaterialTheme.typography.bodySmall,color=MaterialTheme.colorScheme.onSurfaceVariant)
   LazyRow(horizontalArrangement=Arrangement.spacedBy(6.dp)){
    item{AssistChip({vm.setNmeaOutputPreset(NmeaOutputPreset.KC2W_MINIMAL)},label={Text("KC-2W Minimal")})}
    item{AssistChip({vm.setNmeaOutputPreset(NmeaOutputPreset.POSITION_ONLY)},label={Text(tr("Position only","仅船位"))})}
    item{AssistChip({vm.setNmeaOutputPreset(NmeaOutputPreset.HEADING_AND_ROT)},label={Text(tr("Heading + ROT","船首向 + 转向率"))})}
    item{AssistChip({vm.setNmeaOutputPreset(NmeaOutputPreset.FULL_PHONE_SENSORS)},label={Text(tr("Full phone sensors","全部手机传感器"))})}
   }
   Box(Modifier.testTag("nmea_publish_phone_position")){
    SettingSwitch(
     tr("Publish Phone position","发布手机船位"),
     tr("Off by default. Enable only when the boat network needs Phone GNSS. Sends RMC / GGA / VTG / ZDA; Boat position received by the App is never republished.","默认关闭。仅当船载网络需要手机 GNSS 时开启；将发送 RMC / GGA / VTG / ZDA，App 收到的船载船位绝不会被重新发布。"),
     automaticOutput.phonePositionEnabled,
    ){vm.setNmeaPhonePositionPublishing(it)}
   }
   SettingSwitch(tr("Vessel heading (HDT)","船首向（HDT）"),tr("Requires a current bow alignment. If the phone rotates relative to the vessel, realign it; Trip attitude mounting is not required for HDT.","需要当前有效的船艏方向对齐。若手机相对船体发生转动，请重新对齐；发布 HDT 不需要 Trip Watch 的姿态固定确认。"),automaticOutput.phoneHeadingEnabled){vm.setNmeaPhoneHeadingPublishing(it)}
   SettingSwitch(tr("Rate of turn","转向率"),tr("Phone yaw rate while the phone is confirmed vessel-mounted.","手机确认固定于船体时发布偏航转向率。"),automaticOutput.phoneRateOfTurnEnabled){vm.setNmeaPhoneRateOfTurnPublishing(it)}
   SettingSwitch(tr("Heel / Pitch","横倾 / 纵倾"),tr("Trip-confirmed phone attitude; kept separate from rate of turn.","Trip Watch 确认有效的手机姿态，与转向率独立控制。"),automaticOutput.phoneAttitudeEnabled){vm.setNmeaPhoneAttitudePublishing(it)}
   SettingSwitch(tr("Phone pressure","手机气压"),tr("Publishes PHONE_BARO only while the barometer is current.","仅在手机气压计数据当前有效时发布 PHONE_BARO。"),automaticOutput.phonePressureEnabled){vm.setNmeaPhonePressurePublishing(it)}
   SettingSwitch(tr("App-derived wind","App 推算真风"),tr("Off by default. Every dependency must recursively originate from Phone sensors; Boat input is never republished.","默认关闭。所有依赖都必须递归来源于手机传感器；绝不会重新发布船载输入。"),automaticOutput.derivedWindPolicy!=com.yokuli.anchorwatch.domain.vessel.PublicationPolicy.OFF){vm.setNmeaDerivedWindPublishing(it)}
   val estimatedSentences=(if(automaticOutput.phonePositionEnabled)4 else 0)+(if(automaticOutput.phoneHeadingEnabled)if(automaticOutput.phoneHeadingFormat==com.yokuli.anchorwatch.data.vessel.PhoneHeadingOutputFormat.HDT_AND_HDG)2 else 1 else 0)+(if(automaticOutput.phoneRateOfTurnEnabled)1 else 0)+(if(automaticOutput.phoneAttitudeEnabled)1 else 0)+(if(automaticOutput.phonePressureEnabled)1 else 0)+(if(automaticOutput.derivedWindPolicy!=com.yokuli.anchorwatch.domain.vessel.PublicationPolicy.OFF)3 else 0)
   val estimatedCharacters=estimatedSentences*70
   val estimatedBits=estimatedCharacters*10
   Text(tr("Estimated load: $estimatedSentences sentences/s · $estimatedCharacters characters/s · $estimatedBits bits/s · ${"%.0f".format(estimatedBits/48.0)}% of 4800 · ${"%.0f".format(estimatedBits/384.0)}% of 38400","估算负载：$estimatedSentences 条/秒 · $estimatedCharacters 字符/秒 · $estimatedBits bit/s · 占 4800 的 ${"%.0f".format(estimatedBits/48.0)}% · 占 38400 的 ${"%.0f".format(estimatedBits/384.0)}%"),style=MaterialTheme.typography.bodySmall,color=if(estimatedBits>3_840)MaterialTheme.colorScheme.error else MaterialTheme.colorScheme.primary)
   if(estimatedSentences>0)Text(tr("KC-2W NMEA 0183 Output is recommended at 38400 baud. At 4800, several online N2K sources can create conversion backlog.","KC-2W 的 NMEA 0183 Output 建议设为 38400。4800 在多个 N2K 来源在线时可能产生转换积压。"),style=MaterialTheme.typography.bodySmall,color=MaterialTheme.colorScheme.tertiary)
   Text(tr("Never republished: Boat depth, STW, direct Boat wind, Boat position or Boat heading.","绝不重新发布：船载水深、对水航速、船载直接风、船载船位或船载船首向。"),style=MaterialTheme.typography.bodySmall,color=MaterialTheme.colorScheme.onSurfaceVariant)
   Text(tr("Output never auto-starts. Press Start below for each App run so a fragile single-client gateway is not occupied unexpectedly.","输出绝不会自动启动。每次 App 运行都必须在下方明确点击启动，避免意外占用脆弱的单客户端网关。"),style=MaterialTheme.typography.bodySmall,color=MaterialTheme.colorScheme.tertiary)
	  ...
	  item{Card(Modifier.testTag("nmea_output_route")){Column(Modifier.padding(16.dp),verticalArrangement=Arrangement.spacedBy(10.dp)){
	   ...
	   FilterChip(routeMode==NmeaOutputTransportMode.SAME_AS_INPUT_CONNECTION,{routeMode=NmeaOutputTransportMode.SAME_AS_INPUT_CONNECTION},label={Text(tr("Reuse current Boat TCP connection","复用当前船载 TCP 连接"))},enabled=!outputLocked&&state.settings.profile.protocol==com.yokuli.anchorwatch.data.nmea.Protocol.TCP,modifier=Modifier.fillMaxWidth())
	   FilterChip(routeMode==NmeaOutputTransportMode.DEDICATED_TCP,{routeMode=NmeaOutputTransportMode.DEDICATED_TCP},label={Text(tr("Separate TCP output","独立 TCP 输出"))},enabled=!outputLocked,modifier=Modifier.fillMaxWidth())
   Text(if(routeMode==NmeaOutputTransportMode.SAME_AS_INPUT_CONNECTION)tr("Uses the already-open full-duplex Boat NMEA input socket. No second TCP connection is created. A stalled optional write suppresses later TX without closing or reconnecting RX.","使用已经打开的全双工船载 NMEA 输入 Socket，不会再创建第二条 TCP 连接。可选输出若卡住，只会抑制后续 TX，不会关闭或重连 RX。")else tr("Enter a genuinely separate receive address/port for App output. It must not duplicate the safety-owned RX endpoint.","请输入真正独立的 App 输出接收地址 / 端口，不得重复安全监控使用的 RX 端点。"),style=MaterialTheme.typography.bodySmall,color=MaterialTheme.colorScheme.onSurfaceVariant)
	   ...
	   val parsedPort=port.toIntOrNull();val routeValid=if(routeMode==NmeaOutputTransportMode.SAME_AS_INPUT_CONNECTION)state.settings.profile.protocol==com.yokuli.anchorwatch.data.nmea.Protocol.TCP&&state.settings.profile.host.isNotBlank()&&state.settings.profile.port in 1..65535 else host.trim().isNotBlank()&&parsedPort in 1..65535
	   val candidate=when(routeMode){NmeaOutputTransportMode.SAME_AS_INPUT_CONNECTION->automaticOutput.copy(transportMode=routeMode,outputHost="",outputPort=state.settings.profile.port.takeIf{it in 1..65535}?:10110);NmeaOutputTransportMode.DEDICATED_TCP->NmeaOutputEndpointPolicy.tcpDestination(automaticOutput,state.settings.profile,host,parsedPort?:0);else->automaticOutput.copy(transportMode=routeMode,outputHost=host.trim(),outputPort=parsedPort?:0)}
	   if(routeMode!=NmeaOutputTransportMode.SAME_AS_INPUT_CONNECTION)Row(Modifier.fillMaxWidth(),horizontalArrangement=Arrangement.spacedBy(8.dp),verticalAlignment=Alignment.CenterVertically){OutlinedTextField(host,{host=it},Modifier.weight(1f).testTag("nmea_output_tx_host"),singleLine=true,enabled=!outputLocked,label={Text(tr("TX host *","发送主机 *"))},isError=host.isBlank());OutlinedTextField(port,{port=it.filter(Char::isDigit).take(5)},Modifier.width(110.dp).testTag("nmea_output_tx_port"),singleLine=true,enabled=!outputLocked,label={Text(tr("TX port *","发送端口 *"))},isError=parsedPort !in 1..65535,keyboardOptions=KeyboardOptions(keyboardType=KeyboardType.Number))}
   ...
   Text(tr("Provenance firewall: no route forwards or reformats direct Boat input. Byte-identical App TX echoes are labelled and excluded. Transformed/re-encoded input is never claimed or excluded as an exact echo without byte-level proof.","来源防火墙：任何线路都不会转发或改写船载直接输入。字节完全相同的 App TX 回显会被标记并排除；没有逐字节证据时，经过转换或重新编码的输入绝不会被冒充为精确回显或据此排除。"),style=MaterialTheme.typography.bodySmall,color=MaterialTheme.colorScheme.onSurfaceVariant)
  ...
  item{Card(colors=CardDefaults.cardColors(containerColor=if(outputActive)MaterialTheme.colorScheme.primaryContainer else MaterialTheme.colorScheme.surfaceVariant)){Column(Modifier.fillMaxWidth().padding(16.dp),verticalArrangement=Arrangement.spacedBy(8.dp)){
   Row(Modifier.fillMaxWidth(),verticalAlignment=Alignment.CenterVertically){Icon(if(outputActive)Icons.Default.Sensors else Icons.Default.StopCircle,null,tint=if(outputActive)MaterialTheme.colorScheme.primary else MaterialTheme.colorScheme.onSurfaceVariant);Spacer(Modifier.width(10.dp));Column(Modifier.weight(1f)){Text(if(outputActive)tr("Phone/App data is being sent","正在发送手机 / App 数据")else tr("Ready to send Phone/App data","可以发送手机 / App 数据"),fontWeight=FontWeight.SemiBold);Text(if(outputActive&&automaticOutput.transportMode==NmeaOutputTransportMode.SAME_AS_INPUT_CONNECTION)tr("Sending on the existing full-duplex Boat TCP connection; no second socket exists.","正在通过现有全双工船载 TCP 连接发送；没有第二个 Socket。")else if(outputActive)tr("Sending through the saved independent TX route. Boat NMEA input remains independently owned.","正在通过已保存的独立 TX 线路发送；船载 NMEA 输入仍独立管理。")else tr("Choose and save a route above. Saving never starts transmission by itself.","请先选择并保存上方线路；仅保存配置不会自动开始发送。"),style=MaterialTheme.typography.bodySmall,color=MaterialTheme.colorScheme.onSurfaceVariant)}}
   ...
   else Button(vm::startNmeaOutput,Modifier.fillMaxWidth(),enabled=outputReady&&!outputTransitioning){Icon(Icons.Default.PlayArrow,null);Spacer(Modifier.width(6.dp));Text(tr("Start sharing Phone/App data","开始分享手机 / App 数据"))}
   ...
   if(!headingAlignmentReady)Text(tr("Heading stream will wait: align the phone direction to the bow. Other selected independent streams can continue; only ROT, heel and pitch require a confirmed Trip attitude frame.","船首向数据流将等待：请把手机方向与船艏对齐。其他已选的独立数据流仍可继续；只有转向率、横倾和纵倾需要 Trip Watch 中确认姿态参考。"),style=MaterialTheme.typography.bodySmall,color=MaterialTheme.colorScheme.tertiary)
  ...
  item{Card(Modifier.testTag("nmea_output_live_status")){Column(Modifier.fillMaxWidth().padding(14.dp),verticalArrangement=Arrangement.spacedBy(8.dp)){
   ...
   if(pressure!=NmeaWriteBackpressureState.NORMAL)Text(
    when(pressure){
     ...
     NmeaWriteBackpressureState.STALLED->tr("Optional TX is stalled and further TX is suppressed. RX input remains open and is never reconnected by output; Stop and Start output to retry.","可选 TX 已卡住，后续发送已被抑制。RX 输入保持打开，输出不会触发其重连；请停止后再启动输出以重试。")
    },
   ...
   if(showPublisherDiagnostics){
    state.phonePositionOutputStatus.streams.filterKeys{it in setOf("POSITION","HEADING","RATE_OF_TURN","ATTITUDE","PRESSURE","DERIVED_WIND")}.forEach{(stream,status)->
...
@Composable private fun LocalNmeaServicePage(state:MainUiState,vm:MainViewModel,back:()->Unit){
 MarineSubpageTitle(tr("phone nmea service","本机 nmea 服务"))
 ...
 LazyColumn(Modifier.fillMaxSize().padding(16.dp).testTag("local_nmea_server_page"),verticalArrangement=Arrangement.spacedBy(12.dp)){
  ...
  item{Card(colors=CardDefaults.cardColors(containerColor=if(requested)MaterialTheme.colorScheme.primaryContainer else MaterialTheme.colorScheme.surfaceVariant)){Column(Modifier.padding(16.dp),verticalArrangement=Arrangement.spacedBy(8.dp)){
   ...
   if(requested){
    ...
    Text(tr("Screen-off protected: the foreground service holds CPU and both screen-on/screen-off Wi-Fi modes. If Android reclaims the process, this explicit Start is restored during the same device boot. A reboot still requires Start again.","锁屏保护：前台服务会持有 CPU 以及亮屏 / 熄屏两种 Wi-Fi 锁。若 Android 回收进程，本次明确启动会在同一次开机期间自动恢复；设备重启后仍需重新点击启动。"),style=MaterialTheme.typography.bodySmall,color=MaterialTheme.colorScheme.tertiary)
   ...
   }else Button(vm::startLocalNmeaServer,Modifier.fillMaxWidth(),enabled=parsedPort in 1024..65535&&!changed){Icon(Icons.Default.PlayArrow,null);Spacer(Modifier.width(6.dp));Text(tr("Start Phone NMEA service","启动本机 NMEA 服务"))}
   if(!frameReady)Text(tr("The listener may run now. Heading is WAITING_CALIBRATION until bow alignment is completed; Position and Pressure remain available. Trip mounting applies only to ROT, heel and pitch.","监听服务现在可以运行。完成船艏方向对齐前，船首向处于“等待校准”；船位和气压仍可用。Trip Watch 的固定安装只适用于转向率、横倾和纵倾。"),style=MaterialTheme.typography.bodySmall,color=MaterialTheme.colorScheme.tertiary)
...
@Composable private fun outputTransportLabel(value:NmeaOutputTransportMode)=when(value){NmeaOutputTransportMode.SAME_AS_INPUT_CONNECTION->tr("Reused input TCP","复用输入 TCP");NmeaOutputTransportMode.DEDICATED_TCP->tr("Separate TCP output","独立 TCP 输出");NmeaOutputTransportMode.TCP_SERVER->tr("TCP server","TCP 服务器");NmeaOutputTransportMode.UDP_UNICAST->tr("UDP unicast","UDP 单播");NmeaOutputTransportMode.UDP_BROADCAST->tr("UDP broadcast","UDP 广播")}
...
@Composable private fun publisherStreamLabel(value:String)=when(value){"POSITION"->tr("Phone position","手机船位");"HEADING"->tr("Phone vessel heading","手机船首向");"RATE_OF_TURN"->tr("Phone rate of turn","手机转向率");"ATTITUDE"->tr("Phone heel / pitch","手机横倾 / 纵倾");"PRESSURE"->tr("Phone pressure","手机气压");"DERIVED_WIND"->tr("App-derived true wind","App 推算真风");else->tr("Sensor status","传感器状态")}
...
@Composable private fun publisherSuppressionLabel(value:String)=when(value){
 "USER_DISABLED"->tr("disabled by user","用户已关闭");"EXTERNAL_SOURCE_PRESENT","BOAT_SOURCE_PRESENT","TAKEOVER_DELAY","BOAT_SOURCE_LOSS_HOLD","BOAT_SOURCE_HANDOVER"->tr("legacy source-selection state (not used by this publisher)","旧版来源选择状态（当前发布器不再使用）");"PHONE_NOT_MOUNTED"->tr("Trip attitude paused by user","用户已暂停航程姿态");"HEADING_ALIGNMENT_REQUIRED","HEADING_NOT_ALIGNED"->tr("heading alignment not confirmed","尚未确认艏向对齐");"HEADING_ALIGNMENT_EPOCH_MISMATCH"->tr("heading belongs to an older alignment; realign now","船首向属于旧的对齐代次，请立即重新对齐");"HELD_OR_STALE_HEADING"->tr("heading is held rather than newly measured","船首向是保留值，不是新的测量值");"MISSING_PHONE_SENSOR_PROVENANCE"->tr("phone sensor provenance missing","缺少手机传感器来源证明");"SOURCE_DISABLED"->tr("heading source disabled","船首向来源已禁用");"SOURCE_INVALID"->tr("heading source invalid","船首向来源无效");"SOURCE_STALE"->tr("heading source stale","船首向来源已过期");"MOUNT_SUSPECT"->tr("previous attitude frame invalid","上一个姿态参考已失效");"NO_DECLINATION_REFERENCE"->tr("no declination reference","缺少磁偏角参考");"PHONE_HEADING_STALE"->tr("phone heading stale","手机方位已过期");"PHONE_GPS_STALE"->tr("phone GPS stale","手机 GPS 已过期");"NO_APP_DERIVED_WIND","NO_DERIVED_WIND"->tr("App-derived wind unavailable","App 暂时无法推算真风");"OUTPUT_DISCONNECTED"->tr("output disconnected","输出未连接");"SOURCE_CONFLICT"->tr("multiple local candidates; deterministic source selected","多个本机候选；已按固定规则选择");else->value.lowercase().replace('_',' ')
}

@Composable internal fun GpsDataSourceCard(state:MainUiState,vm:MainViewModel){
 val context=androidx.compose.ui.platform.LocalContext.current
 val permissionReady=ContextCompat.checkSelfPermission(context,Manifest.permission.ACCESS_FINE_LOCATION)==PackageManager.PERMISSION_GRANTED
 val permissionLauncher=rememberLauncherForActivityResult(ActivityResultContracts.RequestMultiplePermissions()){grants->
  vm.onPermissionsChanged()
  if(grants[Manifest.permission.ACCESS_FINE_LOCATION]==true)vm.switchGpsDataSource(GpsDataSource.SYSTEM)
  else android.widget.Toast.makeText(context,localized(state.settings.appLanguage,"Precise location is required. Turn on Precise and try Phone GPS again.","需要精确定位权限。请开启精确定位后重试手机 GPS。"),android.widget.Toast.LENGTH_LONG).show()
 }
 val runningWatch=state.active?.paused==false
 val source=if(state.active?.paused==true&&state.settings.gpsDataSource==GpsDataSource.NONE)GpsDataSource.NONE else state.active?.positionSource?.let{runCatching{GpsDataSource.valueOf(it)}.getOrNull()}?:state.settings.gpsDataSource
 val phone=source==GpsDataSource.SYSTEM
 val nmea=source==GpsDataSource.NMEA
 ...
 Column(Modifier.fillMaxWidth(),verticalArrangement=Arrangement.spacedBy(12.dp)){
  Text(tr("Position source","船位来源"),style=MaterialTheme.typography.titleMedium)
  if(state.settings.demoMode) Text(tr("Demo GPS is active. Turn Demo off before choosing a real source.","演示定位已开启，关闭演示后可选择真实来源。")) else {
   GpsSourceRow(tr("Phone GPS","手机 GPS"),tr("Turn on to locate with this device. Turn off to stop phone positioning.","打开即启用本机定位，关闭即停止手机定位。"),phone,!nmea&&!runningWatch&&!proxyActive,"gps_source_system"){
    if(phone)vm.switchGpsDataSource(GpsDataSource.NONE)else if(permissionReady)vm.switchGpsDataSource(GpsDataSource.SYSTEM)else permissionLauncher.launch(arrayOf(Manifest.permission.ACCESS_FINE_LOCATION,Manifest.permission.ACCESS_COARSE_LOCATION))
   }
   HorizontalDivider()
   GpsSourceRow("NMEA",if(connected)tr("Use the connected boat position. Input remains connected when this source is off.","使用已连接船载设备的船位，关闭来源不会断开数据连接。")else tr("Connect NMEA before enabling this source.","请先连接 NMEA，再打开这个来源。"),nmea,!phone&&!runningWatch&&(connected||nmea),"gps_source_nmea"){
    vm.switchGpsDataSource(if(nmea)GpsDataSource.NONE else GpsDataSource.NMEA)
   }
   if(source==GpsDataSource.NONE) Text(tr("Position is off. Choose either source when you need it.","船位来源已关闭，需要时再打开其中一个。"),color=MaterialTheme.colorScheme.onSurfaceVariant)
  }
  if(runningWatch)Text(tr("Pause the anchor watch before changing its position source.","请先暂停锚警，再切换或关闭船位来源。"),color=MaterialTheme.colorScheme.onSurfaceVariant)
  if(proxyActive)Text(tr("Turn off Android GPS proxy before using phone GPS.","使用手机 GPS 前请先关闭 Android 定位代理。"),color=MaterialTheme.colorScheme.onSurfaceVariant)
  if(state.connectionAttempt.state!=ConnectionAttemptState.IDLE)Text(localizeKnownMessage(state.connectionAttempt.message),color=MaterialTheme.colorScheme.error)
 }
}

@Composable internal fun DeveloperSettingsCard(state:MainUiState,vm:MainViewModel){
 val enabled=state.settings.demoMode;val sessionOpen=state.active!=null||state.activeTrip!=null||state.activeSonarSurvey!=null
 Card{Column(Modifier.padding(16.dp),verticalArrangement=Arrangement.spacedBy(10.dp)){
  Text(tr("Demo GPS","演示 GPS"),style=MaterialTheme.typography.titleMedium)
  ...
  HorizontalDivider()
  Text(tr("Build identity","构建来源"),style=MaterialTheme.typography.labelLarge)
  Text("${BuildConfig.VERSION_NAME} (${BuildConfig.VERSION_CODE}) · ${BuildConfig.BUILD_CHANNEL}",fontWeight=FontWeight.SemiBold)
  Text("Git ${BuildConfig.BUILD_GIT_SHA.take(12)}${if(BuildConfig.BUILD_GIT_DIRTY)" · DIRTY" else " · clean"}",style=MaterialTheme.typography.bodySmall,fontFamily=FontFamily.Monospace)
  Text("${BuildConfig.BUILD_GIT_BRANCH} · DB ${BuildConfig.DATABASE_SCHEMA_VERSION} · CI ${BuildConfig.BUILD_IN_CI}",style=MaterialTheme.typography.labelSmall,color=MaterialTheme.colorScheme.onSurfaceVariant)
 }}
}
...
@Composable private fun AlarmBehaviourCard(state:MainUiState,vm:MainViewModel){
 ...
 Card{Column(Modifier.padding(16.dp),verticalArrangement=Arrangement.spacedBy(10.dp)){
  ...
  Text(tr("Anchor and wind warnings use the same global safety-audio path as full alarms. The App loops its built-in two-tone alarm or your custom file; unavailable custom audio falls back to the built-in alarm.","锚泊预警和大风预警与正式警报共用全局安全音频链路。应用会循环播放内置双音警报或你的自定义文件；自定义音频失效时会回退到内置警报。"),style=MaterialTheme.typography.bodySmall,color=MaterialTheme.colorScheme.onSurfaceVariant)
  ...
  Text(tr("Boat Watch uses Android's Alarm audio usage, so normal Silent or Vibrate mode does not mute it. Alarm volume must stay above zero. Do Not Disturb / Total silence can still block alarms unless Android is configured to allow them.","Boat Watch 使用 Android 的“闹钟”音频用途，因此普通静音或振动模式不会将其静音；但闹钟音量必须大于零。若开启“勿扰 / 完全静音”，仍需在 Android 中允许闹钟打断。"),style=MaterialTheme.typography.bodySmall,color=MaterialTheme.colorScheme.onSurfaceVariant)
  ...
  OutlinedButton(vm::openAlarmSoundSettings,Modifier.fillMaxWidth()){Icon(Icons.AutoMirrored.Filled.VolumeUp,null);Spacer(Modifier.width(6.dp));Text(tr("Android sound settings","Android 声音设置"))}
  OutlinedButton(vm::openDoNotDisturbSettings,Modifier.fillMaxWidth()){Icon(Icons.Default.DoNotDisturbOn,null);Spacer(Modifier.width(6.dp));Text(tr("Do Not Disturb settings","勿扰模式设置"))}
  OutlinedButton(vm::openAlarmNotificationSettings,Modifier.fillMaxWidth()){Icon(Icons.Default.NotificationsActive,null);Spacer(Modifier.width(6.dp));Text(tr("Notification settings","通知设置"))}
 }}
}
...
@Composable private fun settingsGpsSourceLabel(source:GpsDataSource):String=when(source){GpsDataSource.NONE->tr("Off","关闭");GpsDataSource.SYSTEM->tr("System GPS","系统 GPS");GpsDataSource.NMEA->"NMEA GPS";GpsDataSource.DEMO->tr("Demo GPS","演示 GPS")}
...
@Composable internal fun PageHeader(title: String, subtitle: String) { Column(verticalArrangement = Arrangement.spacedBy(2.dp)) { if(!LocalMarineEmbeddedRoot.current) Text(title, style = MaterialTheme.typography.headlineSmall, fontWeight = FontWeight.Light); Text(subtitle, color = MaterialTheme.colorScheme.onSurfaceVariant) } }
