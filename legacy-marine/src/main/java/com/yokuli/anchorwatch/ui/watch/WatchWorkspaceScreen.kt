    ...
    ElevatedCard(modifier.testTag("sail_overview_map_preview")){
        Box(Modifier.fillMaxSize()){
            if(BuildConfig.MAPS_CONFIGURED&&(current!=null||points.isNotEmpty())){
                ...
                GoogleMap(Modifier.fillMaxSize(),cameraPositionState=camera,onMapLoaded={loaded=true},uiSettings=MarineMapPolicy.uiSettings(MarineMapContext.SAIL_PREVIEW)){com.yokuli.anchorwatch.map.SharedMarineTileOverlay();
        ...
        Column(Modifier.padding(10.dp),verticalArrangement=Arrangement.spacedBy(6.dp)){
            ...
            if(position==null)Text(tr("Waiting for a Boat or Phone position. Trip recording may continue with gaps.","正在等待船载或手机定位；航程可继续记录，但会保留缺口。"),style=MaterialTheme.typography.bodySmall,color=MaterialTheme.colorScheme.onSurfaceVariant)
                ...
                if(BuildConfig.MAPS_CONFIGURED){
                    ...
                    GoogleMap(Modifier.fillMaxWidth().height(210.dp),cameraPositionState=camera,uiSettings=MapUiSettings(compassEnabled=false,mapToolbarEnabled=false,myLocationButtonEnabled=false,zoomControlsEnabled=false)){com.yokuli.anchorwatch.map.SharedMarineTileOverlay();Marker(state=remember(target){MarkerState(target)},title=tr("Boat","船位"))}
                }
            }
        }
    }
...
internal fun TripStartDialog(state:MainUiState,dismiss:()->Unit,confirmAttitude:(DeviceBowAxis)->Unit={},start:(String,Boolean,VesselSourcePreference)->Unit){
    ...
    val positionPreference=when(state.settings.gpsDataSource){com.yokuli.anchorwatch.domain.model.GpsDataSource.SYSTEM->VesselSourcePreference.PHONE;com.yokuli.anchorwatch.domain.model.GpsDataSource.NMEA->VesselSourcePreference.BOAT;else->VesselSourcePreference.DERIVED}
    ...
    AlertDialog(
        ...
        text={
            Column(verticalArrangement=Arrangement.spacedBy(8.dp)){
                ...
                Text(tr("Recording uses the position source selected in Data. Missing readings remain gaps.","记录使用“数据”中选择的船位来源，缺失读数会保留为空档。"),style=MaterialTheme.typography.bodySmall,color=MaterialTheme.colorScheme.onSurfaceVariant)
