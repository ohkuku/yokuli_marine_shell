...
private fun TripReplayContent(data:TripReplayData){
    ...
    Column(Modifier.heightIn(max=620.dp),verticalArrangement=Arrangement.spacedBy(8.dp)){
        ...
        if(BuildConfig.MAPS_CONFIGURED&&positioned.isNotEmpty()){
            ...
            GoogleMap(Modifier.fillMaxWidth().height(250.dp),cameraPositionState=camera,uiSettings=MapUiSettings(compassEnabled=false,mapToolbarEnabled=false,myLocationButtonEnabled=false,zoomControlsEnabled=false)){com.yokuli.anchorwatch.map.SharedMarineTileOverlay();
