...
internal fun AnchorWatchPage(state: MainUiState, vm: MainViewModel) {
    ...
    val offlineTileProvider=remember(nauticalSource.userChartEnabled,state.offlineMap.revision,com.yokuli.anchorwatch.map.SharedChartLayers.managedByShell){if(nauticalSource.userChartEnabled&&!com.yokuli.anchorwatch.map.SharedChartLayers.managedByShell)vm.createOfflineMapProvider()else null}
    ...
    BottomSheetScaffold(
        ...
        Box(Modifier.fillMaxSize().background(MaterialTheme.colorScheme.surfaceVariant)) {
            if (renderGoogleMap) {
                    ...
                    GoogleMap(Modifier.fillMaxSize(), cameraPositionState = camera, properties = MapProperties(mapType = if(baseMapPolicy.googleBaseMap==GoogleBaseMapKind.SATELLITE)MapType.SATELLITE else MapType.NORMAL,mapStyleOptions=if(baseMapPolicy.applyNauticalStyle)nauticalMapStyle else null), uiSettings = MapUiSettings(compassEnabled = false, indoorLevelPickerEnabled = false, mapToolbarEnabled = false, myLocationButtonEnabled = false, zoomControlsEnabled = false,scrollGesturesEnabled=gesturePolicy.scrollEnabled,zoomGesturesEnabled=gesturePolicy.zoomEnabled,rotationGesturesEnabled=gesturePolicy.rotationEnabled,tiltGesturesEnabled=gesturePolicy.tiltEnabled), onMapLoaded = { mapLoaded = true },onMapClick={point->
                    ...
                    }) {com.yokuli.anchorwatch.map.SharedMarineTileOverlay();
                        ...
                        if(!com.yokuli.anchorwatch.map.SharedChartLayers.managedByShell&&nauticalSource.primary==NauticalPrimarySource.USER_MBTILES&&offlineTileProvider!=null)TileOverlay(tileProvider=offlineTileProvider,fadeIn=false,transparency=(1.0-state.settings.offlineMapOpacity.coerceIn(.30,1.0)).toFloat(),visible=true,zIndex=MapOverlayZ.OFFLINE_CHART)
...
@Composable internal fun localizeSafetyText(value:String):String{
 ...
 return when{
  ...
  value=="The session can start, but movement checks and track recording wait for GPS and the active watch will notify you."->"仍可创建锚泊会话；移动判断与轨迹记录会等待 GPS，监控也会主动通知你。";value=="The session can start from the confirmed anchor coordinate; stale fixes are ignored until live GPS recovers."->"可按已确认锚点坐标创建会话；实时 GPS 恢复前，过期船位不会参与计算。"
  value.startsWith("Fresh · ")->value.replace("Fresh · ","新鲜 · ").replace(" s"," 秒");value=="Unavailable"->"不可用";value=="Position quality is unknown; unsafe fixes remain excluded after the session starts."->"定位质量未知；会话启动后，不安全的船位仍不会参与计算。"
  value=="Accuracy is wider than a useful anchor boundary; the watch will report degraded GPS until it improves."->"定位误差大于实用锚警范围；精度改善前，监控会持续报告 GPS 降级。";value=="Use a wider alarm radius or wait for better accuracy."->"请扩大报警半径，或等待更好的定位精度。";value=="Satellite geometry is weak; allow more margin."->"卫星几何条件较弱，请预留更大余量。"
  ...
  value=="Position unavailable, stale or poor quality"->"船位不可用、已过期或质量不合格";value=="The session can start now; current-generation quality rules still exclude unsafe fixes from movement calculations."->"现在仍可创建会话；当前连接代次与质量规则会继续阻止不安全船位参与移动计算。"
  value=="The session can start now; reconnect and GPS-loss monitoring stays active until a current acceptable position returns."->"现在仍可创建会话；在当前连接恢复合格船位前，重连与 GPS 丢失监控会一直生效。";value=="The session can start now and will notify you that the selected GPS source is unavailable."->"现在仍可创建会话，监控会通知你所选 GPS 数据源不可用。"
  ...
  value=="No active network"->"没有活动网络";value=="The session can start, but NMEA GPS remains degraded until the endpoint is reachable."->"仍可创建会话；在端点恢复可访问前，NMEA GPS 会保持降级状态。";value=="Wi-Fi connected"->"Wi-Fi 已连接";value=="Connected without Wi-Fi"->"已联网但不是 Wi-Fi";value=="Confirm the NMEA server is reachable over this transport."->"请确认可通过当前网络访问 NMEA 服务器。"
...
private fun MapLayersSheet(state:MainUiState,chartState:MapChartUiState,dismiss:()->Unit,setMapType:(Int)->Unit,setUserNautical:(Boolean)->Unit,setLinz:(Boolean)->Unit,setOpacity:(Double)->Unit,setSonar:(Boolean)->Unit,setLinzDepth:(Boolean)->Unit,setPersonalDepth:(Boolean)->Unit,manageChartFiles:()->Unit){
 ...
 ModalBottomSheet(onDismissRequest=dismiss){Column(Modifier.fillMaxWidth().verticalScroll(rememberScrollState()).padding(horizontal=20.dp).padding(bottom=28.dp),verticalArrangement=Arrangement.spacedBy(16.dp)){
  ...
  if(com.yokuli.anchorwatch.map.SharedChartLayers.managedByShell){
   Text(tr("Folder charts are shared across all maps.","文件夹海图在所有地图中共用。"),color=MaterialTheme.colorScheme.onSurfaceVariant)
   OutlinedButton({dismiss();com.yokuli.anchorwatch.map.SharedChartLayers.openLibrary()},Modifier.fillMaxWidth()){Icon(Icons.Default.FolderOpen,null);Spacer(Modifier.width(6.dp));Text(tr("Manage chart layers","管理海图图层"))}
  }
  if(state.settings.mapType==3&&!com.yokuli.anchorwatch.map.SharedChartLayers.managedByShell){
